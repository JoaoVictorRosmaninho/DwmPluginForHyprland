...
SMasterNodeData* CHyprDwmLayout::getNodeFromWindow(PHLWINDOW pWindow) {
...
SMasterWorkspaceData* CHyprDwmLayout::getMasterWorkspaceData(const WORKSPACEID& ws) {
...
SMasterNodeData* CHyprDwmLayout::getMasterNodeOnWorkspace(const WORKSPACEID& ws) {
...
void CHyprDwmLayout::applyNodeDataToWindow(SMasterNodeData* pNode) {
...
void CHyprDwmLayout::switchWindows(PHLWINDOW pWindow, PHLWINDOW pWindow2) {
    ...
    // DWM Logic: When swapping stack with master, the stack becomes master
    // and the old master becomes first in stack (all other stack windows shift down)
    const auto WINDOWSONWORKSPACE = getNodesOnWorkspace(PNODE->workspaceID);
    
    if (WINDOWSONWORKSPACE > 2 && PNODE->isMaster != PNODE2->isMaster) {
        // One is master, one is stack - DWM behavior
        const auto MASTER_NODE = PNODE->isMaster ? PNODE : PNODE2;
        const auto STACK_NODE  = PNODE->isMaster ? PNODE2 : PNODE;
...
void CHyprDwmLayout::buildOrientationCycleVectorFromEOperation(std::vector<eOrientation>& cycle) {
    for (int i = 0; i <= ORIENTATION_CENTER; ++i) {
        cycle.push_back((eOrientation)i);
    }
}

void CHyprDwmLayout::buildOrientationCycleVectorFromVars(std::vector<eOrientation>& cycle, CVarList& vars) {
...
eOrientation CHyprDwmLayout::getDynamicOrientation(PHLWORKSPACE pWorkspace) {
